...
function renderExamples() {
  examplesMount.innerHTML = examplePacks
    .map(
      (pack) => `
...
function downloadPack() {
  const metadata = {
    filename: safeCaseFilename('combined-case'),
  ...
  const guidance = [
    {
      ...
      detail:
        'Combine only packs you control. Remove sensitive personal, medical, or financial details before sharing any output.',
    ...
    },
    {
      ...
      detail:
        'Every combined file should retain the original Current source notes from the source tools.',
      source: 'Open Access UK data provenance',
      url: 'https://github.com/tarunag10/open-access-uk'
    }
  ];
  currentGuidanceMount.innerHTML = guidance
    .map(
      (item) => `
    <article class="card">
      <h3>${item.title}</h3>
