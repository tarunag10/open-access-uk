const PROTOCOL_TYPES = [
  {
    ...
    name: 'Housing Disrepair Protocol',
    responseDays: 90,
    source: 'cpr-practice-direction-pre-action',
    description: 'Claims for disrepair against landlords'
  },
  {
    id: 'debt',
    name: 'Pre-Action Protocol for Debt Claims',
    responseDays: 30,
    source: 'cpr-practice-direction-pre-action',
    description: 'Claims by businesses for payment of debts'
  },
  {
    id: 'personal-injury',
    name: 'Pre-Action Protocol for Personal Injury',
    responseDays: 120,
    source: 'cpr-practice-direction-pre-action',
    description: 'Personal injury claims (RTA, EL, PL)'
  },
  {
    id: 'professional-negligence',
    name: 'Professional Negligence Protocol',
    responseDays: 90,
    source: 'cpr-practice-direction-pre-action',
    description: 'Claims against professionals for negligence'
  }
];
...
export function generateADROffer(data) {
  ...
  const protocolName = protocol
    ? protocol.name.replace('Pre-Action Protocol for ', '').replace(' Protocol', '')
    : data.protocolType;
