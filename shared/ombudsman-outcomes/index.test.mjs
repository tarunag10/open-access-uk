...
test('getOmbudsmanDetails returns full details for housing', () => {
  const details = getOmbudsmanDetails('housing');
  assert.equal(details.id, 'housing');
  assert.equal(details.name, 'Housing Ombudsman');
  assert.ok(details.sectors.includes('Social Housing'));
  assert.equal(details.website, 'https://www.housing-ombudsman.org.uk');
});
...
test('getOutcomeStatistics returns valid statistics for housing', () => {
  const stats = getOutcomeStatistics('housing');
  assert.ok(stats);
  assert.equal(stats.ombudsmanId, 'housing');
  assert.ok(typeof stats.upheldRate === 'number');
  assert.ok(Array.isArray(stats.sectorBreakdown));
});
...
test('getTypicalOutcomes returns outcomes for housing ombudsman', () => {
  const outcomes = getTypicalOutcomes('housing', 'disrepair');
  assert.ok(outcomes);
  assert.ok(Array.isArray(outcomes.outcomes));
});
...
test('getCompensationRanges returns ranges for financial ombudsman', () => {
  const ranges = getCompensationRanges('financial');
  assert.ok(ranges);
  assert.ok(ranges.ranges.length > 0);
  assert.ok(typeof ranges.averageCompensation === 'number');
});
...
test('getDecisionTimescales returns timescales for legal ombudsman', () => {
  const timescales = getDecisionTimescales('legal');
  assert.ok(timescales);
  assert.ok(typeof timescales.averageDays === 'number');
  assert.ok(Array.isArray(timescales.bySector));
});
