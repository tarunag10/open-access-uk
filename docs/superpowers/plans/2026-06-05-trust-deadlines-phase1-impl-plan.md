...
export function formatDateForDisplay(value) {
  /* existing or unified from letter */
}
...
```js
import {
  calculateDeadline as sharedCalc,
  buildICS,
  formatDateForDisplay as sharedFormat
...
icsBtn.onclick = () => {
  /* download the plan.ics or from guidance */
};
```
...
import {
  storageRegistry,
  clearKnownStorage,
  describeStorageRegistry
} from '../shared/privacy/local-storage.mjs'; // adjust path for static deploy (or copy minimal or use relative after copy in build? for Phase1 duplicate array or note path)
```
