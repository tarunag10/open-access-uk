    ...
    const CASE_STATUS = [
      {
        value: 'planning',
        label: 'Planning',
        description: 'Gathering facts and deciding next steps.'
      },
      ...
      return [
        {
          ...
          description:
            'Single Markdown file with the case summary, letters, evidence, and journey.',
...
const { initTheme: initTheme } =
  __m1__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_case_builder_src_theme_js;

const {
  THEME_STORAGE_KEY: THEME_STORAGE_KEY,
  resolveInitialTheme: resolveInitialTheme,
  nextTheme: nextTheme
} = __m2__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_shared_theme_index_mjs;
...
const {
  ...
  cleanJourney: cleanJourney,
  parseCase: parseCase,
  serializeCase: serializeCase,
  parseCaseList: parseCaseList,
  serializeCaseList: serializeCaseList,
  getStatusMeta: getStatusMeta,
  getIssueMeta: getIssueMeta,
  getEvidenceTypeMeta: getEvidenceTypeMeta,
  getJourneyToolMeta: getJourneyToolMeta,
  buildSummary: buildSummary,
  buildStatusBreakdown: buildStatusBreakdown,
  addEvidence: addEvidence,
  removeEvidence: removeEvidence,
  addLetter: addLetter,
  removeLetter: removeLetter,
  addJourneyStep: addJourneyStep,
  removeJourneyStep: removeJourneyStep,
  suggestJourney: suggestJourney,
  buildCaseFile: buildCaseFile,
  parseCaseFile: parseCaseFile,
  buildCaseMarkdown: buildCaseMarkdown,
  buildEvidenceManifest: buildEvidenceManifest,
  buildTimelineMarkdown: buildTimelineMarkdown,
  buildCaseJsonExport: buildCaseJsonExport,
  buildHandoffPack: buildHandoffPack
} = __m3__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_case_builder_src_builder_js;

const {
  buildExportList: buildExportList,
  groupByType: groupByType,
  buildCombinedMarkdown: buildCombinedMarkdown
} = __m4__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_case_builder_src_export_hub_js;
...
function handleAddRepairCase(event) {
  ...
  if (!data.landlordName?.trim()) {
    document.querySelector('#status-msg').textContent =
      'Add the landlord or housing association name.';
    return;
  }
  ...
  const repairRecord = (() => {
    ...
    deadlineDate.setTime(
      deadlineDate.getTime() + (cat[data.repairCategory] || cat.routine) * 60 * 60 * 1000
    );
    return {
      referenceNumber: ref,
  ...
  for (const pattern of dlPatterns) {
    ...
    while ((match = pattern.exec(emailText)) !== null) {
      if (match[2] && /working\s*days?/i.test(match[2])) {
        ...
        const dateStr =
          match[1] || match[2] || match[0].replace(/^(?:by|before|deadline[:\s]*)/i, '').trim();
...
function formatParsedEmailLocal(keyInfo) {
  ...
  if (keyInfo.deadlines.length) {
    const dl = keyInfo.deadlines.map((d) =>
...
function renderEmailPreview(emailText) {
  ...
  if (!emailText || !emailText.trim()) {
    preview.innerHTML =
      '<p class="empty-state">Paste an email above to see extracted information.</p>';
    return;
  }
  const keyInfo = extractKeyInformationLocal(emailText);
  const sections = [];
  ...
  if (keyInfo.dates.length > 0) {
    const dates = keyInfo.dates
      .map(
        (d) => `<li><strong>${escapeHtmlLocal(d.date)}</strong> — ${escapeHtmlLocal(d.event)}</li>`
      )
      .join('');
  ...
  if (sections.length === 0) {
    preview.innerHTML =
      '<p class="empty-state">No key information extracted. Try a more detailed email.</p>';
    return;
  }
  const confidence =
    keyInfo.references.length * 30 +
    (keyInfo.authority ? 25 : 0) +
    keyInfo.deadlines.length * 20 +
    keyInfo.nextSteps.length * 15 +
    keyInfo.dates.length * 10;
...
document
  .querySelector('#email-import-form')
  ?.elements?.namedItem('emailText')
  ?.addEventListener('input', saveEmailDraftOnChange);
restoreEmailDraft();
