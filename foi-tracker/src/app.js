  ...
  (() => {
    ...
    const STATUS_OPTIONS = [
      ...
      { value: 'sent', label: 'Sent', description: 'Request sent, awaiting acknowledgement.' },
      {
        value: 'acknowledged',
        label: 'Acknowledged',
        description: 'Authority has confirmed receipt.'
      },
      {
        value: 'partial',
    ...
    const AUTHORITY_TYPES = [
      ...
      },
      {
        value: 'nhs',
        label: 'NHS body',
        examples: ['trust', 'integrated care board', 'GP practice']
      },
      {
        value: 'police',
        label: 'Police',
        examples: ['constabulary', 'police and crime commissioner']
      },
      {
        value: 'education',
    ...
    function buildActionChecklist(request, today = new Date()) {
      ...
      if (request.status === 'refused') {
        ...
        checklist.push(
          'Consider asking the authority to carry out an internal review of the refusal.'
        );
      }
      ...
      if (needsEscalation(request, today)) {
        checklist.unshift(
          'Request is overdue: consider escalating to an internal review or the ICO.'
        );
      }
      return checklist;
    }
...
const { initTheme: initTheme } =
  __m1__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_foi_tracker_src_theme_js;
...
const { foldIcsLine: foldIcsLine, createIcsEvent: createIcsEvent } =
  __m3__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_shared_calendar_ics_mjs;
...
} = __m4__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_shared_deadlines_index_mjs;

const {
  STATUS_OPTIONS: STATUS_OPTIONS,
  AUTHORITY_TYPES: AUTHORITY_TYPES,
  FOI_RULE: FOI_RULE,
  ICO_COMPLAINT_WINDOW: ICO_COMPLAINT_WINDOW,
  generateRequestId: generateRequestId,
  createRequest: createRequest,
  parseRequest: parseRequest,
  serializeRequest: serializeRequest,
  parseRequestList: parseRequestList,
  serializeRequestList: serializeRequestList,
  getStatusMeta: getStatusMeta,
  getAuthorityTypeMeta: getAuthorityTypeMeta,
  isOverdue: isOverdue,
  computeDeadline: computeDeadline,
  daysUntilDeadline: daysUntilDeadline,
  needsEscalation: needsEscalation,
  buildOverdueList: buildOverdueList,
  buildSummary: buildSummary,
  buildStatusBreakdown: buildStatusBreakdown,
  buildEscalationLetter: buildEscalationLetter,
  buildIcoComplaintLetter: buildIcoComplaintLetter,
  buildReminderEvent: buildReminderEvent,
  buildDeadlineEvent: buildDeadlineEvent,
  buildExportCsv: buildExportCsv,
  buildExportJson: buildExportJson,
  buildHandoffPack: buildHandoffPack,
  buildActionChecklist: buildActionChecklist,
  buildLocalActionPack: buildLocalActionPack
} = __m5__Users_tarunagarwal_Documents_1_App_Developement_Tarun_Open_Access_UK_foi_tracker_src_tracker_js;
